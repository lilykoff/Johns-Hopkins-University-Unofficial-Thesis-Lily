...
% Define abstract command to work with plain page style (no thispagestyle{empty})

% Force page numbers on all chapter pages and redefine abstract to not suppress page numbers
\makeatletter
...
% Redefine abstract environment to NOT use \thispagestyle{empty} and keep normal margins
...
\begin{center}
\vspace*{.25in}
{\bf\LARGE{STATISTICAL METHODS FOR IDENTITY RECOGNITION AND HEALTH PREDICTION USING HIGH-RESOLUTION ACCELEROMETRY DATA}}\\ % change it accordingly, title must be in ALL CAPS!
\vspace*{.75in}
{\bf by} \\*[18pt]
\vspace*{.2in}
{\bf Lily Koffman}\\ % change it accordingly!
\vspace*{1in}
...
{\bf March 2026} \\     % change it accordingly! Put month of submission, not of thesis defense (if different)
\vspace*{.25in}
\begin{small}
{\bf \copyright{ }2026 Lily Koffman} \\ % change the year if needed!
{\bf All rights reserved}
\end{small}
\end{center}
\newpage 

% Add acknowledgements - START CONTINUOUS PAGE NUMBERING
\pagestyle{plain}
\pagenumbering{roman}
\setcounter{page}{2}
...
% RESTART PAGE NUMBERING HERE FOR MAIN CONTENT - ONLY PLACE WHERE NUMBERING RESTARTS
\cleardoublepage % Needed because our intro chapter doesn't really have anything
\pagenumbering{arabic}
\pagestyle{plain}  % Ensure plain page style continues
...
\begin{refsection}[paper1_chapter/paper1_chapter.bib]
\include{paper1_chapter/paper1_chapter}
\printbibliography[heading=bibintoc, title={References}]
\end{refsection}

\begin{refsection}[paper2_chapter/paper2_chapter.bib]
\include{paper2_chapter/paper2_chapter}
\printbibliography[heading=bibintoc, title={References}]
\end{refsection}

\begin{refsection}[paper3_chapter/paper3_chapter.bib]
\include{paper3_chapter/paper3_chapter}
\printbibliography[heading=bibintoc, title={References}]
\end{refsection}
...
\includepdf[pages={-}, pagecommand={\thispagestyle{plain}}, scale=0.9, offset=0 40]{CV_Koffman.pdf} %continue page numbering through CV
